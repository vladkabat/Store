...
const tvs = (state = initialState, action) => {
    ...
    } else if (action.type === SORT_TVS_BY_PRICE_ASC) {
        return {
            ...state,
            content: state.content.slice().sort((tv1, tv2) => {
                let sortVal = 0;
    ...
    } else if (action.type === SORT_TVS_BY_PRICE_DESC) {
        return {
            ...state,
            content: state.content.slice().sort((tv1, tv2) => {
                let sortVal = 0;
                if (tv2.price > tv1.price) {
                    sortVal = 1;
                } else if (tv2.price < tv1.price) {
                    sortVal = -1;
                }
                return sortVal;
            })
